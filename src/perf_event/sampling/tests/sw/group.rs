...
fn test_basic() {
    ...
    let cpu_clock_guard1 = group.add_member(&gen_cfg(SoftwareEvent::CpuClock)).unwrap();
    let cpu_clock_guard2 = group.add_member(&gen_cfg(SoftwareEvent::CpuClock)).unwrap();
...
fn test_guard_basic() {
    let builder = gen_builder();
    let mut group = builder.build_sampling_group().unwrap();
    let mut cpu_clock_guard1 = group.add_member(&gen_cfg(SoftwareEvent::CpuClock)).unwrap();
    let mut cpu_clock_guard2 = group.add_member(&gen_cfg(SoftwareEvent::CpuClock)).unwrap();
