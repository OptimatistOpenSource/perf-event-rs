...
FEATURE := $(shell echo "6.0 5.13 5.12 5.11 5.9 5.8 5.7 5.5 5.4" | tr ' ' '\n' | sort -Vr | awk -v kv="$(KERNEL_MAJOR).$(KERNEL_MINOR)" '{if ($$1 <= kv) {print $$1; exit}}')

build:
	cargo build --no-default-features --features linux-$(FEATURE)

test:
	cargo test --no-default-features --features linux-$(FEATURE)
